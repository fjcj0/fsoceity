...
const CorrectMessage: React.FC<{ type: "posts" | "saves" | "likes" }> = ({
  type,
}) => {
  const {
    getUserContent,
  ...
  if (type === "likes") {
    return user_likes.length === 0 ? (
      <NothingToAppear
        header="No Likes Yet"
        par="You haven't liked any post yet"
      />
    ) : (
      <div className="grid grid-cols-4 max-md:grid-cols-2 gap-3">
        {user_likes.map((liked_post, index) => (
          <ProfileLiked
            key={index}
            id={liked_post.id}
            image={liked_post.post.image}
            par={liked_post.post.content}
          />
        ))}
      </div>
    );
  ...
  if (type === "posts") {
    return user_posts.length === 0 ? (
      <NothingToAppear
        header="No Posts Yet"
        par="You haven't posted anything yet"
      />
    ) : (
      <div className="grid grid-cols-4 max-md:grid-cols-2 gap-3">
  ...
  if (type === "saves") {
    return user_bookmarks.length === 0 ? (
      <NothingToAppear
        header="No Saves Yet"
        par="You haven't saved any post yet"
      />
    ) : (
      <div className="grid grid-cols-4 max-md:grid-cols-2 gap-3">
        {user_bookmarks.map((saved_post, index) => (
          <ProfileSaved
            key={index}
            id={saved_post.id}
            image={saved_post.post.image}
            par={saved_post.post.content}
          />
        ))}
      </div>
    );
...
const page = () => {
  ...
  const [statusEdit, setStatusEdit] = useState<"uneditable" | "editable">(
    "uneditable",
  );
        ...
        <div>
          <button
            ...
            className={`profile-flash active:scale-75 duration-300 ${
              isChangingPicture ? "opacity-50 cursor-none" : ""
            }`}
        ...
        <div className="ml-5 max-md:w-full flex flex-col items-start justify-start gap-y-1 my-auto relative">
          {statusEdit === "editable" ? (
            ...
            <div className="flex flex-col items-start justify-start w-full">
              <h1 className="text-white font-black text-lg md:text-3xl">
                {user?.name}
              </h1>
              <p className="text-white/60 font-light text-sm max-w-[95%] mt-2">
                {user?.bio || "bio"}
              </p>
            </div>
          )}
          {isLoadingUserContents ? (
            <p className="mt-2 text-white/50" style={{ fontSize: 15 }}>
              wait...
            </p>
          ) : (
            <div className="flex mt-2 items-start justify-start gap-4 font-semibold text-lg">
              <p>
                {user_posts.length} <span>Posts</span>
              </p>
              <p>
                {user_bookmarks.length} <span>Saved</span>
              </p>
              <p>
                {user_likes.length} <span>likes</span>
              </p>
            </div>
          )}
